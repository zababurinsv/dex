...
import templates from '/static/html/components/waves-provider/template/index.mjs'
        ...
        constructor () {
            ...
            async function modules (obj) {
               ...
               let account = await (Account())
                        ...
                        if(isEmpty(pass.value)){
                            ...
                            if(pass.value === passConfirm.value){
                                ...
                                let balance = await  events.eventListener.set(true, 'получаем баланс','7',{
                                    views:true,
                                    color:'5',
                                    property:'получить баланс',
                                    substrate:wallet['public']['address'],
                                    relation:'t'
                        ...
                        if(isEmpty(pass.value)){
                         ...
                         let template = await walletTemplate(true,'','3',{
                          ...
                          obj['this'].shadowRoot.querySelector('#wallet').insertAdjacentHTML('beforeend',template)
                            ...
                            obj['this'].shadowRoot.querySelector('button.orders-button').addEventListener('click',async (event)=>{

                                let oneDay = 86400000
                                let timestamp = Date.now() + oneDay
                                let bigNum = new waves.self.BigNumber(timestamp);
                                let result = bigNum.toBytes();
                                let concat = waves.self.concat(wallet.public.key, result)
                                let signature = waves.self.signBytes(wallet.private.seed, concat , 0)

                                let orders = await events.eventListener.set(true, {
                                    timestamp:timestamp,
                                    signature:signature
                                },'7',{
                                ...
                                console.log('_____ orders _____', orders)
                                let orderTemplate =  await templates['orders'](true,'','3',orders,'get template')

                                obj.this.shadowRoot.querySelector('div#orders-list').innerHTML = ''
                                obj.this.shadowRoot.querySelector('div#orders-list').insertAdjacentHTML('beforeend', orderTemplate)
                ...
                events.eventListener.get(true, 'await','4',{}, 'test-wallet',(object)=>{
                    new Promise( async (resolve, reject) =>{
                        let list = new DataTransfer();
                        list.items.add(object['substrate'][`${object['relation']}`]['wallet']);
                        let myFileList = list.files;
                        obj['this'].shadowRoot.querySelector('#account-create').files = myFileList
                        obj['this'].shadowRoot.querySelector('#form-password').value = object['substrate'][`${object['relation']}`]['pass']
                        obj['this'].shadowRoot.querySelector('button.login-button').click()
                ...
                events.eventListener.get(true, 'await','4',{}, 'await-wallet',async (object)=>{
